...
def violin(ax, frame, base, new, pgy_val, pgx_val, config):
    gy = map_config_axis_names(config['plot_gridy'])
    gx = map_config_axis_names(config['plot_gridx'])
    sp = config[config['subplotx']]
    bc = config[config['barcluster']]
    ...
    frame_c = frame.query(generate_query_string({gy: pgy_val, gx: pgx_val, 'config': base}))
    frame_r = frame.query(generate_query_string({gy: pgy_val, gx: pgx_val, 'config': new}))
    data_c = list()
    data_r = list()
    for sbx in sp:
        for bcs in bc:
...
def plot_null_violin(frame, config, base = None, new = None, title = 'Normalized Density'):
    ...
    for i, pgy in enumerate(plotgridy):
        for j, pgx in enumerate(plotgridx):
            gy, gx, sp = map_config_axis_names(config['plot_gridy']), map_config_axis_names(config['plot_gridx']), map_config_axis_names(config['subplotx'])
            violin(axes[i][j], frame, base, new, pgy, pgx, config)    
